...
Modified by Mike Place 3/9/16
** add final as class attribute and add_final()
"""
...
class Job:
    ...
    final(list[strin]): Command and arguments to run as FINAL node for DAGman
    """
    ...
    def add_final(self, path, *args):
        """Add a FINAL node to DAGman
        Args:
        path (string): Path to the script
        *args (optional): Additional arguments to the script
        """
        ...
        for index, arg in enumerate(args):
            self.final.append(arg)

    def add_subdag(self, path, *args):
        """Add a subdag node to DAGman
        Args:
        path (string): Path to the script
        *args (optional): Additional arguments to the script
        """
        self.subdag.append(path)
        for index, arg in enumerate(args):
            self.subdag.append(arg)
