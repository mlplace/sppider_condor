    ...
    def save(self, filename):
        ...
        with open(filename, 'w') as dagfile:
            ...
            for job in self.jobs:
                ...
                if job.name == 'final_node':
                    job_string = 'FINAL %s %s' % (job.name, job.submit_file)
                elif job.name.startswith('subdag'):
                    job_string = 'SUBDAG EXTERNAL %s %s' % (job.name, job.submit_file)
                else:
